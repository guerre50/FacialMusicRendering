...
BOOL EggAvatar::SetCandideAU(const float* AU, const int numberAU)
{
    ...
    if (ret)
    {
		...
		// SOUND: send data to pd
		OSCManager::send("/AU", *AU);
		OSCManager::send("/AUnumber", numberAU);
		OSCManager::send("/lipPosition", AU[0]);
		OSCManager::send("/mouthOpen", AU[1]);
		OSCManager::send("/mouthAmplitude", AU[2]);
		OSCManager::send("/browLevel", AU[3]);
		OSCManager::send("/mouthCurvature", AU[4]);
		OSCManager::send("/browCurvature", AU[5]);
		...
		if (AU[4] > 0.306) {
			if (AU[0] > -0.310) { emotion = "/angry"; value = 1.0f;}
			else {  emotion = "/tender"; value = 1.0f;}
		} else if (AU[3] > 0.074){  emotion = "/angry"; value = 0.6f;}
		else if (AU[0] > 0.08) { emotion = "/happy"; value = 0.8f;}
		else if (AU[1] > 0.152){  emotion = "/sad"; value = 0.7f;}
		else if (AU[2] > 0.150) { emotion = "/sad"; value = 0.5f;}
		else if (AU[4] > 0.166) { emotion = "/neutral"; value = 1.0f;}
		else if (AU[4] > 0.148) { emotion = "/tender"; value = 1.0f;}
		else if (AU[4] <= 0.148) { emotion = "/neutral"; value = 1.0f;}
		
		OSCManager::send("/emotion", emotion);
		//OSCManager::send(emotion, value);
        ...
        if (numberAU < 8 || (AU[6] == 0 && AU[7] == 0))
        {
			...
			//OSCManager::send("/emotionDetected", emotion);
        }
...
BOOL EggAvatar::SetRotations(const float pitchDegrees, const float yawDegrees, const float rollDegrees)
{
	...
	ss  << pitchDegrees << " " <<  yawDegrees << " " <<  rollDegrees;
	std::string res = ss.str();

	OSCManager::sendRotation(yawDegrees, pitchDegrees, rollDegrees);
